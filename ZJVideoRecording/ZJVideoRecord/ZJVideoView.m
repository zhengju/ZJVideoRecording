...
@interface ZJVideoView()<ZJModelDelegate>
...
@property (nonatomic, strong, readwrite) ZJModel *fmodel;


@end


@implementation ZJVideoView
...
#pragma mark - view
- (void)BuildUIWithType:(ZJVideoViewType)type
{
    
    self.fmodel = [[ZJModel alloc] initWithFMVideoViewType:type superView:self];
...
- (void)startRecord
{
    if (self.fmodel.recordState == ZJRecordStateInit) {
        [self.fmodel startRecord];
    } else if (self.fmodel.recordState == ZJRecordStateRecording) {
...
{
    if (recordState == ZJRecordStateInit) {
        [self updateViewWithStop];
        [self.progressView resetProgress];
    } else if (recordState == ZJRecordStateRecording) {
